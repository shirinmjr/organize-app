...
const InputSelect = ({ label, name, options, value = "", onChange = null }: InputSelectProps) => {
  const [selected, setSelected] = useState(options.find((option) => option.option_id === value));
        ...
        <Listbox
          ...
          <div className="p-2 bg-white border border-blue-400 rounded-full outline-none ring-0 focus:ring-1">
            ...
            <Listbox.Options className="absolute p-2 mt-4 border border-blue-400 shadow cursor-pointer min-w-48 max-w-10/12 focus:outline-none bg-blue-50 max-h-60 rounded-md">
              {options.map((option) => (
                <Listbox.Option
                  className={({ active }) => `relative cursor-pointer select-none p-2 ${active ? "bg-blue-100" : ""}`}
